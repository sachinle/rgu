<?php
/**
 * Patents — RGU Research Portal
 * -----------------------------
 * Data pulled from research/data/patents.php (centralised).
 * Same UI shell & CSS styling as grants-received.php, publications.php & book-chapters.php —
 * hero + sticky year picker + dark-header table + live search filter.
 */
$PATENTS = require __DIR__ . '/data/patents.php';
...
foreach ($PATENTS as $y) {
    $totalRows += count($y['rows']);
    foreach ($y['rows'] as $r) {
        $st = strtolower($r[4] ?? '');
        if (strpos($st, 'granted') !== false) {
            $grantedCount++;
        } elseif (strpos($st, 'published') !== false) {
            $publishedCount++;
        } else {
            $filedCount++;
        }
...
<head>
    ...
    <meta name="description" content="Patents filed, published, and granted to faculty, researchers, and student innovators at Rathinam Global University.">
    <link rel="icon" type="image/png" href="../assets/images/favicon.png">
    <title>Patents &amp; IP | Research · Rathinam Global University</title>
    ...
    <style>
        ...
        .g-table tbody td.title{font-weight:700;color:#0f172a;min-width:240px;}
        .g-table tbody td.inventors{color:#334155;min-width:200px;}
        .g-table tbody td.appno{white-space:nowrap;color:var(--brand);font-weight:600;font-family:monospace;font-size:12px;}
        .g-table tbody td.date{white-space:nowrap;color:#475569;font-weight:500;}
        ...
        .status-badge {
            ...
            font-size: 11px; font-weight: 700; text-transform: uppercase; tracking: .04em;
            white-space: nowrap;
        }
        .status-granted { background: #ecfdf5; color: #047857; border: 1px solid #a7f3d0; }
        .status-published { background: #f0f9ff; color: #0369a1; border: 1px solid #bae6fd; }
        .status-filed { background: #faf5ff; color: #7e22ce; border: 1px solid #e9d5ff; }
...
<section class="g-hero">
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <h1 class="font-sora text-4xl sm:text-5xl lg:text-6xl font-extrabold leading-[1.05] tracking-tight max-w-4xl">
            <span class="grad">Patents &amp; IP</span> — Technological Innovations &amp; Design Grants
        </h1>
        <p class="mt-5 max-w-2xl text-base sm:text-lg text-white/75 leading-relaxed">
            A complete year-wise index of utility patents, design patents, and intellectual property filed, published, and granted to faculty, researchers, and student innovators at Rathinam Global University.
        </p>

        <!-- Summary strip -->
        <div class="mt-10 grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4 max-w-3xl">
            <div class="g-stat rounded-2xl px-5 py-4">
                <p class="font-sora text-3xl font-black leading-none"><?= count($PATENTS) ?></p>
            ...
            <div class="g-stat rounded-2xl px-5 py-4">
                ...
                <p class="text-[10px] font-semibold uppercase tracking-widest text-white/55 mt-1">Total Patents</p>
            </div>
            <div class="g-stat rounded-2xl px-5 py-4">
                <p class="font-sora text-3xl font-black leading-none"><?= $grantedCount ?></p>
                <p class="text-[10px] font-semibold uppercase tracking-widest text-white/55 mt-1">Patents Granted</p>
            </div>
            <div class="g-stat rounded-2xl px-5 py-4">
                <p class="font-sora text-3xl font-black leading-none"><?= $publishedCount ?></p>
                <p class="text-[10px] font-semibold uppercase tracking-widest text-white/55 mt-1">Published Patents</p>
            </div>
        </div>
    </div>
</section>
...
<section class="py-10 sm:py-14">
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div class="g-search max-w-md mx-auto mb-8">
            ...
            <input id="g-filter" type="search" placeholder="Search by title, inventor, status, application no or school…">
        </div>
        ...
        <?php foreach ($PATENTS as $i => $y): ?>
            <div class="yr-panel <?= $i === 0 ? 'is-active' : '' ?>" data-year-panel="<?= $i ?>">
                <div class="g-card">
                    <div class="g-card-head">
                        ...
                        <span class="total-pill">
                            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"><path d="M15 14c.2-1 .7-1.7 1.5-2.5 1-.9 1.5-2.2 1.5-3.5A6 6 0 0 0 6 8c0 1 .2 2.2 1.5 3.5.7.7 1.3 1.5 1.5 2.5"/><path d="M9 18h6"/><path d="M10 22h4"/></svg>
                            <?= count($y['rows']) ?> patents
                        </span>
                    </div>
                        ...
                        <table class="g-table">
                            <thead>
                                <tr>
                                    ...
                                    <th>Inventors / Faculty</th>
                                    <th>Status</th>
                                    <th>Application / Design No.</th>
                                    <th>Submitted Date</th>
                                </tr>
                            </thead>
                            <tbody>
                                <?php foreach ($y['rows'] as $r):
                                    // $r = [S.No, School, Title, Inventors, Status, Application No, Submitted Date]
                                    $r = array_pad($r, 7, '');
                                    $st = strtolower($r[4]);
                                    $badgeClass = 'status-filed';
                                    if (strpos($st, 'granted') !== false) {
                                ...
                                ?>
                                    <tr>
                                        ...
                                        <td class="title"><?= htmlspecialchars($r[2]) ?></td>
                                        <td class="inventors"><?= htmlspecialchars($r[3]) ?></td>
                                        <td>
                                            <span class="status-badge <?= $badgeClass ?>">
                                                <?= htmlspecialchars($r[4]) ?>
                                            </span>
                                        </td>
                                        <td class="appno"><?= htmlspecialchars($r[5]) ?></td>
                                        <td class="date"><?= htmlspecialchars($r[6]) ?></td>
                                    </tr>
                                <?php endforeach; ?>
                            </tbody>
                        </table>
                ...
                <div class="g-empty hidden" data-empty>
                    ...
                    <p class="font-sora font-bold text-slate-700">No patents match your search</p>
